...
### Simulation on full graph

After comparing metrics between networks, we will compare the spread of news in these networks following a SI epidemic model. The simulation is performed in C++, and it outputs the ratio of infected nodes at each time step.

```{r}
FNN.infected_ratio = as.numeric(scan("results/SI_infection_rate_FNN.csv", sep=',', what = "numeric", quiet = TRUE))
TNN.infected_ratio = as.numeric(scan("results/SI_infection_rate_TNN.csv", sep=',', what = "numeric", quiet = TRUE))

plot(1:length(FNN.infected_ratio),FNN.infected_ratio, type='o')
line(1:length(TNN.infected_ratio),TNN.infected_ratio, type='+')
```

### Simulation on initially infected graph

SIS threshold evaluation
```{r}
graphs <- vector("list", 2)
graphs[[1]] <- (FNN_only_infected)
graphs[[2]] <- (TNN_only_infected)
names <- c('FNN_only_infected', 'TNN_only_infected')
gamma <- 0.2
p0 <- 0.05
iters <- 100
result <- threshold_evalutation(graphs,names,gamma,p0,iters)

# Table
result %>% group_by(group) %>% summarize(eigen=round(1/mean(inv_lambda), 2), inv_lambda=mean(inv_lambda), beta_gama=mean(beta_gama))

# Plot
ggplot(data=result, aes(x=time, y=sim, group=group, color=as.factor(group))) +
  geom_line(data=result %>% dplyr::filter(grepl('above', group)), aes(x=time, y=sim, group=group, color=as.factor(group)), linetype="dashed") +geom_line(data=result %>% dplyr::filter(grepl('below', group)), aes(x=time, y=sim, group=group, color=as.factor(group))) +
  xlab("Time") +
  ylab("Infected nodes (%)")
```
...
# Multiple news spreaders graph

Now we will analyze two different networks, which will also call FNN and TNN. 
This time, FNN is a graph that contains spreader users across multiple fake news.
TNN represents the same but for true news.

```{r}
# Initialize fake news network
FNN.2 <- read.graph("graph_FNN.txt", format="edgelist", directed=FALSE)

# Initialize true news network
TNN.2 <- read.graph("graph_TNN.txt", format="edgelist", directed=FALSE)
```
...
degrees <- degree(FNN.2)
FNN.2.degree.histogram <- as.data.frame(table(degrees))
FNN.2.degree.histogram[,1] <- as.numeric( paste(FNN.2.degree.histogram[,1]))
network_degree_summary <- c('FNN.2',vcount(FNN.2),
                   max(FNN.2.degree.histogram$degrees),
                   sum(FNN.2.degree.histogram$degrees)/vcount(FNN.2),
                   vcount(FNN.2)/sum(FNN.2.degree.histogram$degrees))
degree_summary[nrow(degree_summary) + 1,] = network_degree_summary

degrees <- degree(TNN.2)
TNN.2.degree.histogram <- as.data.frame(table(degrees))
TNN.2.degree.histogram[,1] <- as.numeric( paste(TNN.2.degree.histogram[,1]))
network_degree_summary <- c('TNN.2',vcount(TNN.2),
                   max(TNN.2.degree.histogram$degrees),
                   sum(TNN.2.degree.histogram$degrees)/vcount(TNN.2),
                   vcount(TNN.2)/sum(TNN.2.degree.histogram$degrees))
degree_summary[nrow(degree_summary) + 1,] = network_degree_summary
...
```{r}
degrees_df <- data.frame()
degrees_df <- rbind(degrees_df, cbind(FNN.2.degree.histogram,
                                        group='FNN.2'))
degrees_df <- rbind(degrees_df, cbind(TNN.2.degree.histogram,
                                        group='TNN.2'))
...
network_clustering_summary <- c('FNN.2',
                              transitivity(FNN.2),
                              transitivity(FNN.2, "localaverage"))
clustering_summary[nrow(clustering_summary) + 1,] = network_clustering_summary

network_clustering_summary <- c('TNN.2',
                              transitivity(TNN.2),
                              transitivity(TNN.2, "localaverage"))
clustering_summary[nrow(clustering_summary) + 1,] = network_clustering_summary
...
```{r}
FNN.2.infected_ratio = as.numeric(scan("results/SI_infection_rate_FNN.2.csv", sep=',', what = "numeric", quiet = TRUE))
TNN.2.infected_ratio = as.numeric(scan("results/SI_infection_rate_TNN.2.csv", sep=',', what = "numeric", quiet = TRUE))

plot(1:length(FNN.2.infected_ratio),FNN.2.infected_ratio, type='o')
line(1:length(TNN.2.infected_ratio),TNN.2.infected_ratio, type='+')
```